...
public class Day05 : Day
{
    ...
    {
        var (stacks, instructions) = GetStacksAndInstructions(input);
    ...
    public override object Part2(List<string> input)
    {
        ...
        {
            var keepOrder = Enumerable.Range(0, instr.CratesToMove).Select(_ => instr.From.Pop()).Reverse();
            keepOrder.ForEach(i => instr.To.Push(i));
        });
        return string.Join("", stacks.Select(s => s.Pop()));
    }


    private (Stack<char>[] Stacks, IEnumerable<MoveInstruction> Instructions) GetStacksAndInstructions(List<string> input)
    {
